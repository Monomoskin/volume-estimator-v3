...
def process_sample_pair(predictor, metadata, sample_key, input_image_dir, output_base_dir, category_names):
    """
    Procesa un par de imágenes TOP y SIDE, maneja múltiples clases de interés y agrega debug.
    """
    ...
    if not os.path.exists(top_path) or not os.path.exists(side_path):
        ...
        side_path = os.path.join(input_image_dir, side_name)
        if not os.path.exists(top_path) or not os.path.exists(side_path):
            print(f"\n[SKIP] Faltan archivos para la muestra {sample_key}.")
            return
    ...
    # Todas las clases de interés en TOP
    ...
    # Mostrar qué clases detectó SIDE
    detected_classes_side = [category_names[i] for i in instances_side.pred_classes.tolist()]
    print("  [DEBUG] Clases detectadas SIDE:", detected_classes_side)

    frasco_side_instance = find_highest_score_instance(instances_side, frasco_side_id)
    if frasco_side_instance is None:
        print("  [ERROR] Contenedor no detectado en SIDE. Se guardará debug visual.")
        # Guardar imagen debug SIDE
        ...
        debug_path = os.path.join(output_base_dir, f"{sample_key}_SIDE_debug.jpg")
        cv2.imwrite(debug_path, out_side.get_image()[:, :, ::-1])
        print(f"  [DEBUG SAVE] Imagen SIDE debug guardada en: {debug_path}")
        return
    ...
    print(f"  [SIDE] Factor Z: {factor_z_mm_per_pixel:.6f} mm/px")
    ...
    detected_classes_top = [category_names[i] for i in instances_top.pred_classes.tolist()]
    print("  [DEBUG] Clases detectadas TOP:", detected_classes_top)

    frasco_top_instance = find_highest_score_instance(instances_top, frasco_top_id)
    if frasco_top_instance is None:
        print("  [ERROR] Contenedor no detectado en TOP. Guardando debug visual.")
        ...
        debug_path = os.path.join(output_base_dir, f"{sample_key}_TOP_debug.jpg")
        cv2.imwrite(debug_path, out_top.get_image()[:, :, ::-1])
        return
    ...
    print(f"  [TOP] Factor XY: {pixels_to_mm2:.6f} mm²/px²")

    # --- Cálculo de Volumen ---
    volume_results = []
    for idx in range(len(instances_top)):
        current_class_id = instances_top.pred_classes[idx].item()
        if current_class_id in top_classes_of_interest:
    ...
    # Guardar imágenes TOP y SIDE con predicciones
    ...
    print(f"  [SAVE] CSV guardado para {sample_key} ({len(volume_results)} células).")
...
def main():
    ...
    model_path = "output_train/model_0001199.pth" 
